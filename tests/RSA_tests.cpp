...
TEST(RSAPublicKey, constructor)
{
	// Small case
	{
		...
		Crypto::RSAPublicKey expected(3, 3);

		res = Crypto::Utils::from_hex("3006020103020103", data, data_sz);
		EXPECT_EQ(res, 0);

		EXPECT_EQ(Crypto::RSAPublicKey(data, data_sz), expected);
	}

	// Case from actual PEM file
	{
		int res;
		uint8_t data[512];
		std::size_t data_sz = sizeof(data);
...
TEST(RSAPublicKey, constructor_abnormal)
{
	...
	// Data is longer than sequence size
	{
		...
		res = Crypto::Utils::from_hex("3003020103FF", data, data_sz);
		EXPECT_EQ(res, 0);
	...
	// Second item is not an integer
	{
		...
		res = Crypto::Utils::from_hex("3006020103010100", data, data_sz);
		EXPECT_EQ(res, 0);
...
TEST(RSAPublicKey, to_binary)
{
	// Small case
	{
		...
		uint8_t data[256];
		std::size_t data_sz = sizeof(data);
		std::string pubKey_str, expected = "3006020103020103";

		Crypto::RSAPublicKey pubKey(3, 3);
...
{
	// Small case with Carmichael function
	{
		int res;
		uint8_t data[256];
		std::size_t data_sz = sizeof(data);
	...
	// Case from actual PEM file (OpenSSL uses Euler totient for d)
	{
		int res;
		uint8_t data[2048];
		std::size_t data_sz = sizeof(data);
...
{
	// Case #1: Not enough space to write (small with Carmichael)
	{
		int res;
		uint8_t data[256];
		std::size_t data_sz = sizeof(data);
	...
	// Case #3: Not enough space to write (large from OpenSSL PEM file)
	{
		int res;
		uint8_t data[2048];
		std::size_t data_sz = sizeof(data);
...
{
	std::vector<std::vector<std::string>> tests = {
		{ "00", "03", "07", "05", "0B", "03", "07", "01", "false" }, // N == 0
		{ "02", "03", "07", "05", "0B", "03", "07", "01", "false" }, // N is even
		{ "37", "01", "07", "05", "0B", "03", "07", "01", "false" }, // E == 1
		{ "37", "02", "07", "05", "0B", "03", "07", "01", "false" }, // E is even
		{ "37", "37", "07", "05", "0B", "03", "07", "01", "false" }, // E == N
		{ "37", "03", "07", "01", "0B", "03", "07", "01", "false" }, // P == 1
		{ "37", "03", "07", "02", "0B", "03", "07", "01", "false" }, // P is even
		{ "37", "03", "07", "05", "01", "03", "07", "01", "false" }, // Q == 1
		{ "37", "03", "07", "05", "02", "03", "07", "01", "false" }, // Q is even
		{ "37", "03", "01", "05", "0B", "03", "07", "01", "false" }, // D == 1
		{ "37", "03", "37", "05", "0B", "03", "07", "01", "false" }, // D == N
		{ "4D", "03", "37", "05", "0B", "03", "07", "01", "false" }, // N != P * Q
		{ "37", "03", "07", "05", "0B", "00", "07", "01", "false" }, // DP == 0
		{ "37", "03", "07", "05", "0B", "03", "00", "01", "false" }, // DQ == 0
		{ "37", "03", "07", "05", "0B", "03", "07", "00", "false" }, // QP == 0
		// TODO
		// (d * e) - 1   % (p-1) != 0
		// (d * e) - 1   % (q-1) != 0
		// (dp - d)      % (p-1) != 0
...
{
	// TODO: p not prime, q not prime, p & q not prime, p & q prime
}
