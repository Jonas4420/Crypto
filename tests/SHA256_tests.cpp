	...
	for ( auto file : files ) {
		...
		for ( auto tests : test_vectors ) {
			for ( auto test : tests ) {
				...
				Crypto::MessageDigest_get<Crypto::SHA256>(msg.get(), msg_sz, md);
...
TEST(SHA256, MonteCarlo)
{
	int res;
	uint8_t seed[Crypto::SHA256::SIZE];
	uint8_t m[4][Crypto::SHA256::SIZE];
	std::size_t md_sz = Crypto::SHA256::SIZE;
	std::string md_str;

	std::string file_path = TestOptions::get().vect_dir + "SHA/" + "SHA256Monte.rsp";
	...
	for ( auto tests : test_vectors ) {
		...
		for ( auto test : tests ) {
			...
			for ( std::size_t i = 3 ; i < 1003 ; ++i ) {
				Crypto::MessageDigest_get<Crypto::SHA256>(m[0], 3 * md_sz, m[3]);
	...
	for ( auto file : files ) {
		...
		for ( auto tests : test_vectors ) {
			for ( auto test : tests ) {
				...
				std::size_t msg_sz = atoi(test["Len"].c_str()) / 8;
				std::unique_ptr<uint8_t[]> msg(new uint8_t[msg_sz]);
				uint8_t md_1[Crypto::SHA256::SIZE];
				uint8_t md_2[Crypto::SHA256::SIZE];
