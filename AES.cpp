...
AES::set_keyenc(const uint8_t *key, std::size_t key_sz)
{
	...
	switch ( nr ) {
		case 10:
			for( std::size_t i = 0; i < 10 ; ++i, RK += 4 ) {
				RK[4]  = RK[0] ^ RCON[i]
...
AES::set_keydec(const uint8_t *key, std::size_t key_sz)
{
	...
	memcpy(buf_tmp, buf_enc, sizeof(buf_enc));
	rk_tmp = buf_tmp;
	...
	*RK++ = *SK++;
	...
	SK -= 8;
	for ( std::size_t i = nr - 1 ; i > 0; --i, SK -= 8 ) {
		for ( std::size_t j = 0 ; j < 4 ; ++j, ++SK ) {
			*RK++ =   RT0[FSb[(*SK      ) & 0xFF]]
				^ RT1[FSb[(*SK >>  8) & 0xFF]]
				^ RT2[FSb[(*SK >> 16) & 0xFF]]
				^ RT3[FSb[(*SK >> 24) & 0xFF]];
...
const uint32_t AES::FT0[256] = { FT };
#undef V

#define V(a,b,c,d) 0x##b##c##d##a
const uint32_t AES::FT1[256] = { FT };
#undef V

#define V(a,b,c,d) 0x##c##d##a##b
const uint32_t AES::FT2[256] = { FT };
#undef V

#define V(a,b,c,d) 0x##d##a##b##c
const uint32_t AES::FT3[256] = { FT };
#undef V

#undef FT

const uint8_t AES::RSb[256] = {
...
#define V(a,b,c,d) 0x##a##b##c##d
const uint32_t AES::RT0[256] = { RT };
#undef V

#define V(a,b,c,d) 0x##b##c##d##a
const uint32_t AES::RT1[256] = { RT };
#undef V

#define V(a,b,c,d) 0x##c##d##a##b
const uint32_t AES::RT2[256] = { RT };
#undef V

#define V(a,b,c,d) 0x##d##a##b##c
const uint32_t AES::RT3[256] = { RT };
#undef V
