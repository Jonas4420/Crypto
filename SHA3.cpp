...
SHA3::update(const uint8_t *input, std::size_t input_sz)
{
	...
	while ( input_sz >= (fill = r - buffer_sz) ) {
		absorb((uint8_t*)state, buffer_sz, input, fill);
		keccakf(state);
	...
	if ( input_sz > 0 ) {
		absorb((uint8_t*)state, buffer_sz, input, input_sz);
		buffer_sz += input_sz;
	}
}
...
SHA3::finish(uint8_t *output)
{
	...
	// Set domain specification and padding
	s[buffer_sz] ^= 0x06;
	s[r - 1] ^= 0x80;
...
void
SHA3::absorb(uint8_t *data, std::size_t offset, const uint8_t *mask, std::size_t mask_sz)
{
