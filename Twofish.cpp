...
#define Q(x, a, b, c, d, l)                        \
{                                                  \
	(x)[0] = q[(a)][(x)[0]] ^ GET_BYTE((l),0); \
	(x)[1] = q[(b)][(x)[1]] ^ GET_BYTE((l),1); \
	(x)[2] = q[(c)][(x)[2]] ^ GET_BYTE((l),2); \
	(x)[3] = q[(d)][(x)[3]] ^ GET_BYTE((l),3); \
}

#define ENCROUND(i, a, b, c, d)                    \
{                                                  \
	F0  = g(a);       F1  = g(ROL((b), 8));    \
	F0 += F1;         F1 += F0;                \
	F0 += K[2*(i)+8]; F1 += K[2*(i)+9];        \
	(c) = ROR((c) ^ F0, 1);                    \
	(d) = ROL((d), 1) ^ F1;                    \
}
...
#define ENCCYCLE(i)                                \
{                                                  \
	ENCROUND(2*(i),   R0, R1, R2, R3);         \
	ENCROUND(2*(i)+1, R2, R3, R0, R1);         \
}

#define DECROUND(i, a, b, c, d)                    \
{                                                  \
	F0  = g(a);       F1  = g(ROL((b), 8));    \
	F0 += F1;         F1 += F0;                \
	F0 += K[2*(i)+8]; F1 += K[2*(i)+9];        \
...
#define DECCYCLE(i)                                \
{                                                  \
	DECROUND(2*(i)+1, R2, R3, R0, R1);         \
	DECROUND(2*(i),   R0, R1, R2, R3);         \
}
...
uint32_t
Twofish::h(uint8_t x, const uint32_t *L, std::size_t k) const
{
	uint8_t y[4];
	h0(x, L, k, y);
	return mds[0][y[0]] ^ mds[1][y[1]] ^ mds[2][y[2]] ^ mds[3][y[3]];
}

uint32_t
Twofish::g(uint32_t X) const
{
	return    mds[0][s[0][GET_BYTE(X, 0)]] ^ mds[1][s[1][GET_BYTE(X, 1)]]
		^ mds[2][s[2][GET_BYTE(X, 2)]] ^ mds[3][s[3][GET_BYTE(X, 3)]];
}
...
Twofish::RS_Mod(uint32_t c)
{
	...
	uint32_t c1 = c2 ^ (c >> 1) ^ ((c & 0x01) ? (mod >> 1) : 0);

	return (c1 << 24) | (c2 << 16) | (c1 << 8) | c;
}
...
const uint32_t Twofish::mds[4][256] = {
	{
