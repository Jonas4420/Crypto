...
void
SHA512::update(const uint8_t *input, std::size_t input_sz)
{
	std::size_t fill;
	uint32_t left;
	...
	left = (uint32_t)(total[0] & 0x7F);
	fill = 128 - left;

	total[0] += (uint64_t)input_sz;

	if ( total[0] < (uint64_t)input_sz ) {
		total[1]++;
	}
...
void
SHA512::finish(uint8_t *output)
{
	std::size_t last, padn;
	uint64_t high, low;
...
void
SHA512::process(const uint8_t data[128])
{
	std::size_t i;
